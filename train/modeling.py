...
def export_model_fn_builder(config: GroverConfig, init_checkpoint):
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        sequence_length = tf.reduce_sum(sequence_mask, axis=-1)[0]
...
def initialize_from_context(initial_context, ignore_ids, news_config, 
    ...
    return {
        ...
        'probs': tf.concat([tf.exp(gt_logprobs), context_output['new_probs'][:, None]], axis=1)
    }
...
def sample(news_config: GroverConfig, initial_context, eos_token, min_len, max_len, ignore_ids=None, 
    ...
    with tf.name_scope('sample_sequence'):
        ...
        def cond(ctx, cache, probs):
            ...
            return tf.logical_not(tf.logical_and(is_eos, length_cond))
